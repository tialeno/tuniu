...
//             exit;
        ...
        if(count($search_gid)>0){
            // 有商品id
            $goods = DB::table('goods')->whereIn('id',$search_gid)->paginate(10);
        }else{
            // 没商品id 到分类表里查询
            // 获得分类id
            ...
            if(count($search_gid2)>0){
                // 有商品id
                $goods = DB::table('goods')->whereIn('id',$search_gid2)->paginate(10);
            }else{
                // 没商品id 模糊查询商品表
                
                // 获取条件的个数
                if(empty($arr2)){
                    $goods = DB::select('SELECT * FROM goods WHERE title like "%'.$keyword.'%" LIMIT 10');
                }else{
                    $where = '';
                    foreach($arr2 as $value3){
                        $where .= ' and title like "%'.$value3.'%"';
                    }
                    $goods = DB::select('SELECT * FROM goods WHERE title like "%'.$keyword.'%"'.$where.' LIMIT 10');
                }
